...
## Bản copy này trong WChem

Thư mục `classic-lab/` là **bản vendor đầy đủ source** của chem_lab (git clone
nguồn, trừ `node_modules`/`.git`), kèm `dist/` là bản build sẵn với
`--base=/classic-lab/`.

- WChem serve `/classic-lab` từ `classic-lab/dist/` (mount trong
  `taskflow/backend/src/app.js`, trước static chung).
- Muốn cập nhật theo upstream:
  ```bash
  cd classic-lab && npm install && npm run build -- --base=/classic-lab/ && cp -r dist/. dist/.tmp  # rồi copy dist/ ra ngoài
  ```
  Đơn giản hơn: build rồi thay toàn bộ `dist/` bằng bản mới, commit.
- Mọi lần deploy lên Render phải đảm bảo `Dockerfile` có `COPY classic-lab classic-lab`
  (đã có).
